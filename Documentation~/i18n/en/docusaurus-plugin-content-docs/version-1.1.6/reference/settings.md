...
5. HotUpdateOnly restores that published-client snapshot and never mixes in new local AOT output. An actual payload mismatch blocks by default; the Release window can authorize a one-shot force publication after a high-risk confirmation and records it in the report.
...
## FTP is not part of Settings

`QHYFrameworkSettings` contains no `ftpHost`, `ftpPort`, `ftpUserName`, or password
field. FTP is an editor-publication concern owned exclusively by the QHY Release
window. Non-secret values are local per-project/platform EditorPrefs; Windows stores
the remembered password in Credential Manager. CI uses `QHY_FTP_PASSWORD` and
optional `QHY_FTP_USERNAME`. None of these values enter the Settings asset.
