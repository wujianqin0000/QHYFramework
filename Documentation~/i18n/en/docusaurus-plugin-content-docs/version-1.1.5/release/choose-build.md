...
It restores the Full Package snapshot for that client and verifies every file's
SHA-256. Any changed AOT Metadata bundle blocks publication and requires a new
Full Package. A local AOT difference is still a compatibility warning because
it is excluded from this hot update.
...
Before building, save all scenes, select the correct platform, ClientVersion,
and a new ResourceVersion, ensure remote roots contain no version, inspect the
