...
analyzes the real YooAsset dependency collection and generates Engine Stripping
protection, builds GamePackage and bundle delta/size/upload reports, copies StreamingAssets,
...
## External-content Engine Stripping protection

After the final Collector configuration, FullPackage performs dependency-aware
collection with `simulateBuild: false`; it does not reuse the simulation pass that
intentionally omits dependencies. The analysis covers every main collected asset and
its YooAsset dependencies. It currently recognizes `.anim`, `AnimationClip`, `Motion`,
runtime animator controllers, animator override controllers, avatars/avatar masks, and
imported models that contain animation clips.

When external animation content is found, QHY overwrites
`Assets/Game/Generated/QHYLink/link.xml` with:
...
The file is synchronously imported before YooAsset and `BuildPipeline.BuildPlayer`.
QHY never edits `Assets/HybridCLRGenerate/link.xml`, so a later HybridCLR Generate/All
cannot erase this protection. With `PlayerSettings.stripEngineCode == true`, a missing,
malformed, or partial animation-module entry blocks FullPackage publication instead of
allowing an Android IL2CPP client whose bundled animations may not play. The release
report audits `stripEngineCode`, `engineStrippingLinkPath`,
`engineStrippingProtectedAssemblies`, `externalAnimationAssetCount`, and
`externalAnimationAssets`. QHY still
generates the file while engine stripping is disabled so re-enabling stripping does not
depend on a manual linker edit.
...
enter both manually. Ensure resource BaseURLs contain no platform folder or version, inspect the
