...
The target is prepared once under the shared
`Library/QHYFrameworkPublisher/Staging` workspace: one deterministic,
cache-preferred `npm ci`, followed by one bilingual production build. GitHub and
Gitee both copy this validated staging package and do not reinstall dependencies
or rebuild documentation independently.
