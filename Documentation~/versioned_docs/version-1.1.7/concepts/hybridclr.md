...
- **link.xml**：告诉 Unity Linker/IL2CPP 保留 Player 中的托管类型或 Engine 模块；它不提供
  HybridCLR 泛型补充元数据，补充元数据也不能把已经裁掉的 Engine 运行时代码放回客户端。
...
## link.xml 与补充元数据不能互相替代

外置 YooAsset Bundle 不参加 Unity 内置场景的静态类型扫描。若 Boot 没有引用动画，而
AnimatorController/AnimationClip 只存在于 Bundle，Engine Stripping 可能裁掉
`UnityEngine.AnimationModule` 的运行时支持。FullPackage 会为这种情况生成独立的
`Assets/Game/Generated/QHYLink/link.xml`；不要修改会被 Generate/All 覆盖的
`Assets/HybridCLRGenerate/link.xml`。

`UnityEngine.AnimationModule.dll` 仍可按项目需要加入“额外 AOT 元数据程序集”，用于热更
代码访问相应 AOT 类型的元数据场景，但这只是 HybridCLR 元数据处理，不能代替
`<assembly fullname="UnityEngine.AnimationModule" preserve="all" />` 的 Player 裁剪保护。
