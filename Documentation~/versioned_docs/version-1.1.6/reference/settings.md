...
5. HotUpdateOnly 始终恢复对应已发布客户端的冻结快照，不会混入当前工程的新 AOT 输出；实际负载不匹配默认阻止发布，Release 窗口可在高风险确认后对当前一次发布强制放行并写入报告。

切换平台后如果显示分析平台不一致，请为当前平台执行 `Generate/All` 或点击“刷新”。“刷新”只重新读取已有生成结果，不执行耗时的 HybridCLR 生成。

## FTP 不属于 Settings

`QHYFrameworkSettings` 不包含 `ftpHost`、`ftpPort`、`ftpUserName` 或密码字段。FTP 是
QHY Release 窗口独占的编辑器发布配置：非敏感项按项目/平台保存在本机 EditorPrefs，密码
默认进入 Windows 凭据管理器。CI 使用 `QHY_FTP_PASSWORD` 和可选的
`QHY_FTP_USERNAME`。这些值不会写入 Settings Asset。
