...
`-resourceVersion` 省略时启用资源版本自动化：根据已发布基线、本地 Releases 和 YooAsset
输出生成下一个未占用修订。编辑器窗口的单一“版本自动化”开关还会同时接管 ClientVersion；
命令行的客户端版本仍由 `-clientVersion` 明确指定。生产 CI 推荐由发布编排器明确分配
ResourceVersion，避免并发任务竞争同一个修订号。
