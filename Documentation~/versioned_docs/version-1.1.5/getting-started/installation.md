...
YooAsset Collector 配置。默认 FTP 主机和用户名为空，各平台客户端版本从
`v1.0.0` 开始。

:::warning 不要提交凭据
本次变更不再把 FTP Password 写进项目资产。只在 Release 窗口临时输入，或通过
`QHY_FTP_PASSWORD`/CI Secret 注入。不要把密码放入 URL、命令行或日志。
:::
...
之后才按 `upload-plan.json` 增量上传。HotUpdateOnly 必须找到 Full Package 的客户端和
